        ...
        <div :class="$style[`${clsPrefix}${clsName}-cityCon`]">
          ...
          <div :class="$style[`${clsPrefix}${clsName}-cityList`]">
            <div :class="$style[`${clsPrefix}${clsName}-cityItem`]" v-for="(item,index) in currentList" :key="index">
              <div :class="[$style[`${clsPrefix}${clsName}-citynumber`],
  ...
  export default {
    ...
    watch: {
      cityWorkerFlowList: function() {
        this.formatData(this.cityWorkerFlowList);
      }
    },
    mounted() {
      if (this.init === true) {
        this.initData(this.cityWorkerFlowList);
      }
    },
    methods: {
      ...mapMutations(["setCityWorkerFlowType"]),
      initData() {
        this.setCityWorkerFlowType(this.currentNav);
        this.formatData(this.cityWorkerFlowList);
      },
      formatData(data = []) {
        let list = [];
        if (data.length > 0) {
          if (this.currentNav === this.navList[0].value) {
            ...
            this.currentList = list
              .slice(0, 15)
              .concat(list.slice(list.length - 15, list.length));
            this.progressMax = this.currentList[0].value2;
          }
        }
      },
