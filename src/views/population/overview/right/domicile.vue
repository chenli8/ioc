...
export default {
  ...
  mounted() {
    ...
    if (this.init === true || this.countyLive.length > 0) {
      this.initData();
    }
  },
  methods: {
     ...
     */
    formatData(data = []) {
      let dataList = convertArrayData(data, { nameKeys: ['liveCountyName'], valueKeys: ['rate'] }, { label: 'liveCountyName' });
      if (dataList.length > 0) {
        //时间轴选中与数据dateTime一致
        this.timeline.value = dataList[0].dateTime;
        delete this.chart.config.color;
        delete this.chart.other.shadow.formatter;
    ...
    changeTime(data) {
      ...
      this.updateCountyLive({ dateTime: this.timeline.value });
    }
  },
  beforeDestroy() {
  }
