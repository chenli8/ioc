...
import { getIcon, getBarConfig, getBarSeriesConfig, GRADIENTCOLORS, COLORS } from '@/business-component/chart/chart.config.js';
export default {
  props: {
    config: {
      type: Object,
  ...
  methods: {
    renderChart() {
      ...
      for (let i = 0; i < legendDataLen; i++) {
        let seriesConfig = getBarSeriesConfig();
      ...
      if (config.tooltip && config.tooltip.show && config.tooltip.formatter === undefined) {
        ...
        config.tooltip.formatter = (par) => {
          ...
          par.forEach((p, index) => {
            ...
            if (p.name === p.axisValueLabel) {
              str = `${icon}${p.axisValueLabel}: ${p.value} ${units[p.seriesIndex] || ''}<br/>`;
            } else {
