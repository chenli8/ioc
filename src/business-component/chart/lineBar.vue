...
import { getIcon, getLineBarConfig, getLineBarSeriesConfig, GRADIENTCOLORS, COLORS } from '@/business-component/chart/chart.config.js';
export default {
  props: {
    config: {
      type: Object,
  ...
  methods: {
    renderChart() {
      ...
      for (let i = 0; i < legendDataLen; i++) {
        let seriesConfig = getLineBarSeriesConfig();
        const otherSeriesConfig = Array.isArray(this.other.series) ? this.other.series[i] || {} : this.other.series || {};
        //用户未主动配置type=(bar|line) 并且 数据为最后一项则默认使用line
        if (otherSeriesConfig.type === undefined && i === legendDataLen - 1) {
      ...
      let barCount = series.filter((item) => { return item.type === 'bar' });
      //reset legendIcon、
      series.forEach((item, index) => {
        const legendItem = legendData[index];
        if (item.type === 'line') {
          item.smooth = true;//平滑过渡
          legendItem.icon = '';
          item.yAxisIndex = 1;
        } else {
          legendItem.textStyle.padding = [0, 0, 0, -7];
        }
        //计算柱子宽度比例
        let barWidth = parseInt(item.barWidth || 45);
        item.barWidth = `${(barWidth / barCount.length)}%`;
      });
      //set tooltip
