<template>
  <Axis :series="chart.series" :config="chart.config"></Axis>
</template>
<script>
import hexToRgba from '@/lib/hexToRgba.js';
import { merge, createChartTemp, units } from '@/lib/util.js';
import Axis from '@/components/chart/axis.vue';
import { getHorizontalBarConfig, getHorizontalBarSeriesConfig, getBarShadowConfig, GRADIENTCOLORS, COLORS } from '@/business-component/chart/chart.config.js';
export default {
  props: {
    config: {
      type: Object,
      default: () => {
        return {};
      }
    },
    other: {
      type: Object,
      default: () => {
        return
      }
    },
    data: {
      type: Array,
      default: () => {
        return [];
      }
    }
  },
  data() {
    return {
      chart: createChartTemp()
    }
  },
  watch: {
    data: {
      handler() {
        this.renderChart();
      },
      deep: true
    }
  },
  components: { Axis },
  mounted() {
  },
  methods: {
    init() {
    },
    renderChart() {
      const data = this.data;
      let config = getHorizontalBarConfig();
      let datas = [];
      let _datas = [];//记录实际值
      let yAxisData = [];
      let series = [];
      let shadowData = [];
      let maxVal = 0;
      const firstItem = data[0];
      //处理负数
      const minus = this.other.minus;
      merge(config, this.config);

      // set legend
      let legendData = [];
      let legendDataLen = 0;
      let legendNames = [];

      for (let key in firstItem) {
        const name = firstItem[`${key}`];
        if (name !== undefined && key.indexOf('name') !== -1 && key !== 'name') {
          legendNames.push(name);
        }
      }
      legendDataLen = legendNames.length;

      for (let i = 0; i < legendDataLen; i++) {
        const name = legendNames[i];
        let item = merge({
          name,
          icon: 'circle',
          textStyle: {
            color: config.color[i * 2] || COLORS[0]
          }
        }, config.legend.data[i] || {});

        legendData.push(item);
      }
      // set datas
      for (let i = 0; i < legendDataLen; i++) { datas.push([]) }
      for (let i = 0; i < data.length; i++) {
        const item = data[i];
        yAxisData.push(item.label || '');
        //图例数组长度代表数据分组有多少个
        for (let l = 0; l < legendDataLen; l++) {
          let val = item[`value${l + 1}`];
          if (val !== undefined) {
            _datas.push(val);
            if (minus && val < 0) {
              val = 0;
            }
            maxVal = Math.max.call(null, maxVal, val);
            datas[l].push(val);
          }
        }
      }
      for (let i = 0; i < legendDataLen; i++) {
        let seriesConfig = getHorizontalBarSeriesConfig();
        const otherSeriesConfig = Array.isArray(this.other.series) ? this.other.series[i] || {} : this.other.series || {};
        merge(seriesConfig, otherSeriesConfig);
        seriesConfig.label.formatter = (par) => {
          return _datas[par.dataIndex] + (seriesConfig.label.unit !== undefined ? seriesConfig.label.unit : units[0]);
        }
        //设置渐变色系
        const startColor = config.color[i * 2] || GRADIENTCOLORS[0];
        const endColor = config.color[i * 2 + 1] || GRADIENTCOLORS[1];
        seriesConfig.itemStyle.color.colorStops[0].color =startColor.indexOf('rgb') > -1 ? startColor : hexToRgba(startColor, 1);
        seriesConfig.itemStyle.color.colorStops[1].color = endColor.indexOf('rgb') > -1 ? endColor : hexToRgba(endColor, 1);
        seriesConfig.name = legendData[i]['name'];
        seriesConfig.data = datas[i];
        series.push(seriesConfig);
      }
      //判断是否有阴影缺少shadow配置项或者配置项存在 show=true
      if (this.other && (this.other.shadow === undefined || this.other.shadow.show)) {
        //阴影值 (最大值为0表示所有数据有可能全部为0(用于缺少数据，只显示阴影只为UI效果))
        let shadowConfig = getBarShadowConfig();
        merge(shadowConfig, this.other.shadow || {});
        shadowData = Array(series[0] ? series[0].data.length : 0).fill(maxVal);
        shadowConfig.data = shadowData;
        for (let s = 0; s < series.length; s++) {
          const item = series[s];
          if (item.label) {
            //关闭标签显示
            item.label.show = false;
            //真实值显示在阴影上(UI效果)
            shadowConfig.label.formatter = this.other.shadow && this.other.shadow.formatter ? this.other.shadow.formatter : (par) => {
              const val = _datas[par.dataIndex];
              return `${val}${item.label.unit !== undefined ? item.label.unit : units[0]}`;
            }
          }
        }
        series.unshift(shadowConfig);

      }
      config.yAxis[0].data = yAxisData;
      this.chart.config = config;
      this.chart.series = series;
     
    }
  }
}
</script>

